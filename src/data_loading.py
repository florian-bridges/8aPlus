...
class BoulderDataLoader():
    ...
    def __init__(self, batch_size, split=[0.8, 0.1, 0.1], seed=1337):
        ...
        train_tensor, validation_tensor, test_tensor = random_split(TensorDataset(X, y), split)
        self.train_loader =  DataLoader(train_tensor, batch_size=batch_size, shuffle=True)
        self.validation_loader =  DataLoader(train_tensor, batch_size=batch_size, shuffle=True)
